...
class RobertaModel(FairseqEncoderModel):
    ...
    def forward(self, src_tokens, features_only=False, return_all_hiddens=False, classification_head_name=None,
                target_mask=None, parallel_data_mask=None, parallel_data=None, **kwargs):
        if classification_head_name is not None:
            features_only = True

        # if parallel_data_mask is not None, extra is masked_target_logits
        x, extra = self.encoder(src_tokens, features_only, return_all_hiddens, **kwargs)
        if parallel_data_mask is not None and parallel_data is not None:
            extra, _ = self.encoder(parallel_data, False, return_all_hiddens, masked_tokens=parallel_data_mask)

        if classification_head_name is not None:
            x = self.classification_heads[classification_head_name](x, target_mask=target_mask)
        return x, extra
...
class RobertaClassificationHead(nn.Module):
    ...
    def forward(self, features, target_mask=None, **kwargs):
        if target_mask is None:
            x = features[:, 0, :]  # take <s> token (equiv. to [CLS])
        else:
            x = features[target_mask, :]  # K x C (K=\sum_i B_i)
        x = self.dropout(x)
        x = self.dense(x)
        x = self.activation_fn(x)
        x = self.dropout(x)
